 ...
 * @LastEditTime: 2022-02-26 15:34:19
 */
...
public:
    ...
    // void reverse() { mystl::reverse(begin(), end()); }   // BUG: error: ‘reverse’ is not a member of ‘mystl’
...
vector<T>& vector<T>::operator=(const vector& rhs) {
    if (this != &rhs) {
        ...
        if (len > capacity()) {
            vector tmp(rhs.begin(), rhs.end());     // BUG: 
            // error: no matching function for call to ‘mystl::vector<int>::vector(mystl::vector<int>::const_iterator, mystl::vector<int>::iterator)’
            swap(tmp);
        } else if (size() >= len) {
            auto i = mystl::copy(rhs.begin(), rhs.end(), begin()); //BUG:
            // error: no matching function for call to ‘copy(mystl::vector<int>::const_iterator, mystl::vector<int>::iterator, mystl::vector<int>::iterator)’
            data_allocator::destroy(i, end_);
            end_ = begin_ + len;
        } else {
            mystl::copy(rhs.begin(), rhs.begin() + size(), begin_); // BUG:
            //error: no matching function for call to ‘uninitialized_copy(mystl::vector<int>::const_iterator, mystl::vector<int>::iterator, mystl::vector<int>::value_type*&)’
...
vector<T>::insert(const_iterator pos, const value_type& value) {
    ...
    if (end_ != cap_ && xpos == end_) {
        ...
        mystl::copy_backward(value_copy);
...
void vector<T>::swap(vector<T>& rhs) noexcept {
    ...
    template <class T>
    bool operator<(const vector<T>& lhs, const vector<T>& rhs) {
        return mystl::lexicographical_compare(lhs.begin(), lhs.end(), rhs.begin(), rhs.end());  //BUG: 原来有 bug
    }
