...
#define MYTINY_ALGOBASE_H_

/**
 * @file algobase.h
 * @author yufeng (you@domain.com)
 * @brief 此文件包含mystl的基本用法
 * @version 0.1
 * @date 2022-02-25
 * 
...
typename std::enable_if<
    ...
    Up*>::type unchecked_copy_backward(Tp* first, Tp* last, Up* result) {
        ...
        if (n != 0) {
            result -= n;
            std::memmove(result, first, n * sizeof(Up)); // TODO: memmove() ? 将first移动到result的位置上？
        }
        return result;
    }
...
/*********************************************************************************************************
 * copy_if
 * 把[first, last)内 满足一元操作 unary_pred 的元素拷贝到以 result 为起始的位置上
 ********************************************************************************************************/
template <class InputIter, class OutputIter, class UnaryPredicate>  // TODO: UnaryPredicate?
...
/*********************************************************************************************************
 * copy_n
 * 把 [first, first + n)区间上的元素拷贝到 [result, result+) 上
 * 返回一个pair 分别指向拷贝结束的尾部
 *********************************************************************************************************/
...
template <class InputIter, class OutputIter>
OutputIter move(InputIter first, OutputIter last, OutputIter result) {
 ...
 * 以 字典序 排列对两个进行比较，当在某个位置发现第一组不相等时，有下列几种情况：
 * 1）如果第一序列的元素较小，返回true，否则返回 false
...
bool lexicographical_compare(const unsigned char* first1,
    const unsigned char* last1, const unsigned char* first2, const unsigned char* last2) {
