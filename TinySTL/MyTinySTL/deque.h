...
#define DEQUE_MAP_INIT_SIZE 8
    ...
    struct deque_iterator : public iterator<random_access_iterator_tag, T> {
        ...
        typedef T*          value_pointer;      // 指向成员数据(即 cur, first, last)的指针
        typedef T**         map_pointer;        // 声明缓冲区所在节点
        ...
        // 后加加   (返回类型是self，模拟int类型，比如int a, a++++是不允许的。但++++a是可以的)
        self operator++(int) {
            self tmp = *this;
            ++*this;
            return tmp;
        ...
        self& operator+=(difference_type n) {
            ...
            if (offset >= 0 && offset < static_cast<difference_type>(buffer_size)) {
                ...
                // 要跳到其他的缓冲区
                const auto node_offsete = offset > 0 // > 0 表示在后面的缓冲区， <= 表示前面的缓冲区（当n为负数时）
        ...
        // 只有成员函数都有this指针， 友元函数没有this指针（因为它不是类的成员）
        // this指针 是成员函数的隐含参数，可以用来指向调用对象。
    ...
    public:
        // TODO: implements about iterators as follows
        // 迭代器相关操作
        ...
        iterator    insert(iterator position, value_type value&& value);
    ...
    private:
        ...
        void            destroy(map_pointer nstart, map_pointer nfinish);
    ...
    map_init(size_type nElem) {
        ...
        begin_.set_node(nstart);    // 设定begin iterator的node, first, last指针指向
    ...
    fill_init(size_type n, const value_type& value) {
        map_init(n);
        if (n != 0) {
            for (auto cur = begin_.node; cur < end_.node; ++cur) {  // 先填充buffer都占满的 
                mystl::uninitialized_fill(*cur, *cur + buffer_size, value);
            }
            mystl::uninitialized_fill(end_.first, end_.cur, value); // 再填充剩下的
        }
    }
    ...
    copy_init(FIter first, FIter last, forward_iterator_tag) {
        ...
        for (auto cur = begin_.node; cur < end_.node; ++cur) {
            auto next = first;  // 找到buffer_size后的下一个位置
            mystl::advance(next, buffer_size);
            mystl::uninitialized_copy(first, next, *cur); // 每次复制buffer_size个元素
            first = next;
        }
    ...
    fill_insert(iterator position, size_type n, const value_type& value) {
        ...
        if (elems_before < (len / 2)) { // TODO: 前一半后一半？ 
            ...
            try {   
                if (elems_before >= n) {
                    ...
                    begin_ = new_begin;
                    myst::copy(begin_n, position, old_begin);
                }
            } catch (...) {
    ...
    template <class FIter>
    void deque<T>::
    insert_dispatch(iterator position, FIter first, FIter last, forward_iterator_tag, forward_iterator_tag) {
        if (last <= first) return ;
        const size_type n = mystl::distance(first, last);
        if (position.cur == begin_.cur) {
            ...
            try {
                ...
                if (new_begin.node != begin_.node) {
                    destroy_buffer(new_begin, begin_.node - 1);
                }
                throw;
            }
    ...
    template <class T>
    void deque<T>::
    require_capacity(size_type n, bool front) {
    ...
    template <class T>
    void deque<T>::
    reallocate_map_at_front(size_type need_buffer) {
        ...
        // 另新的 map 中的指针指向原来的buffer，并开辟新的buffer
        auto begin = new_map + (new_map_size_ - new_buffer) / 2;
    ...
    template <class T>
    void deque<T>::
    reallocate_map_at_back(size_type need_buffer) {
        const size_type new_map_size = mystl::max(map_size_ << 1,
                                                map_size_ + need_buffer + DEQUE_MAP_INIT_SIZE);
        map_pointer new_map = create_map(new_map_size);
        const size_type old_buffer = end_.node - begin_.node + 1;
        ...
        // 另新的 map 中的指针指向原来的 buffer， 并开辟新的buffer
